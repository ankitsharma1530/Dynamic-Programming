...
public:
    ...
    int solve(vector<int>& piles,int ind, int m){
        ...
        // we had traverse our array upto n-4th index, we are having only 4 cells left              to check
        // now see if our player found those 4 cells into its range(i.e from 1 to 2*m)
        // he will consume all those to make its profit max (because we had to choose              optimally)
        ...
        for(int i=1;i<=m*2;i++){
            ...
            // as if this will be maximum then only we will win the game
    ...
    int stoneGameII(vector<int>& piles) {
        ...
        // this value is basically the diffrence between the profit of player A and B
