    ...
    // check in notes for recursive calls
    // but here no recursive calls are used 
    // this can done without any use of dp
    // as it is a simple subarray problem
    // hence we can solve it by using for_loops
    ...
    int numberOfArithmeticSlices(vector<int>& A) {
        ...
        for(int i=1;i<n-1;++i){
            ...
            if(curr_diff==last_diff){ // if last difference == current difference
                ++curr; 
                // then "A[i+1]" also contribute to the last arithmetic sequence
            }
            else{
